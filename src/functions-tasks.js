...
function getPolynom(...args) {
  ...
  if (len === 3) {
    res = (x) => args[0] * x ** 2 + args[1] * x + args[2];
  }
  if (len === 2) res = (x) => args[0] * x + args[1];
  if (len === 1) res = () => args[0];
...
function logger(func, logFunc) {
  return (...args) => {
    const argums = JSON.stringify(args).slice(1, -1);
    logFunc(`${func.name}(${argums}) starts`);
    const res = func(...args);
    logFunc(`${func.name}(${argums}) ends`);
